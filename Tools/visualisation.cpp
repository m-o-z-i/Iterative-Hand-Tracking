...
void drawHand(cv::Mat& img, const std::shared_ptr<Hand >& hand, cv::Scalar color) {
    ...
    // arm direction
    cv::arrowedLine(img, hand->mPosition, hand->mPosition + cv::Point2i(30 * hand->mArmDirection), color, 2, 8, 0, 0.3);
    ...
    for (const std::shared_ptr<Blob> blob : hand->mBlobs){
        if(blob->mClass == FINGER_THUMB){
            ...
            cv::circle(img, blob->mFilteredPosition, 5, color,1);
        } else {
        ...
        // line to arm center
        //cv::line(img, hand->mArmEllipse.mPosition, blob->mPosition, cv::Scalar(255,255,255));
...
void drawHands(cv::Mat& img, const std::vector<std::shared_ptr<Hand> >& hands, cv::Scalar color, bool rndcolor){
    cv::RNG rng(54321);
    for (const std::shared_ptr<Hand> hand : hands){
        cv::Scalar colorRnd = cv::Scalar(rng.uniform(0,255), rng.uniform(0, 255), rng.uniform(0, 255));
        if(rndcolor)
            drawHand(img, hand, colorRnd);
        else
            drawHand(img, hand, color);
    }
}
