...
variation_(std::numeric_limits<double>::infinity()), parent_(), child_(nullptr), next_(nullptr)
{
    std::fill_n(moments_, 5, 0.0);
}
...
void MSER::process(const uint8_t * bits, int width, int height)
{
    ...
    std::vector<bool> accessible(width * height);
    std::vector<int> boundaryPixels[256];
	int priority = 256;
    std::vector<std::shared_ptr<Region>> regionStack;
...
void MSER::processStack(int newPixelGreyLevel, int pixel, std::vector<std::shared_ptr<Region>> & regionStack)
{
...
std::ptrdiff_t MSER::doublePool(std::vector<std::shared_ptr<MSER::Region>> & regionStack)
{
    assert(!pool_.empty()); // Cannot double the size of an empty pool

    std::vector<Region> newPool(pool_.size() * 2);
    std::copy(pool_.begin(), pool_.end(), newPool.begin());
